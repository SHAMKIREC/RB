...
const mapReview = (row) => ({ id: row.id, clientName: row.client_name, location: row.location, serviceTitle: row.service_title, reviewText: row.review_text, rating: Number(row.rating || 5), photos: row.photos || [], orderNumber: row.order_number || '', contact: row.contact || '', consent: Boolean(row.consent), publishReview: Boolean(row.publish_review), publishName: Boolean(row.publish_name), publishLocation: Boolean(row.publish_location), publishPhotos: Boolean(row.publish_photos), status: row.status || 'pending', isPublished: row.is_published ?? true, isDemo: Boolean(row.is_demo), createdAt: row.created_at, updatedAt: row.updated_at });
...
export async function saveReview(review) {
  ...
  if (!admin) throw new Error('Создание отзывов доступно только администратору.');
  if (review.isPublished && review.adminPublicationConsent !== true) {
    throw new Error('Подтвердите разрешение заказчика на публикацию текста и фотографий.');
  }
