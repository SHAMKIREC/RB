...
export default function Header({ theme, onToggleTheme }) {
  ...
  const calculatorTimer = useRef(null);
  const [menuOpen, setMenuOpen] = useState(false);
  const [menuRendered, setMenuRendered] = useState(false);
  const [desktopSosOpen, setDesktopSosOpen] = useState(false);
  const [calculatorOpening, setCalculatorOpening] = useState(false);
  const isActive = (path) => location.pathname === path || location.pathname.startsWith(`${path}/`);

  const closeMenu = () => setMenuOpen(false);

  const openCalculator = () => {
    if (calculatorOpening) return;
    setCalculatorOpening(true);
    calculatorTimer.current = window.setTimeout(() => {
      navigate("/calculator");
      setCalculatorOpening(false);
    }, 460);
  };

  useEffect(() => () => window.clearTimeout(calculatorTimer.current), []);
        ...
        <div className="flex h-16 items-center justify-between gap-2 sm:h-20 sm:gap-4">
          ...
          <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
            ...
            <button
              ...
              aria-busy={calculatorOpening}
              title="Калькулятор сметы"
              className={`calculator-quick-action relative grid h-10 w-10 shrink-0 place-items-center overflow-hidden rounded-xl border text-white/80 transition-all hover:bg-white/10 hover:text-white active:scale-90 min-[900px]:hidden ${isActive('/calculator') ? 'border-white/35 bg-white/10 text-white' : 'border-white/20'} ${calculatorOpening ? 'is-opening' : ''}`}
            >
              <span className="calculator-quick-ring" aria-hidden="true" />
              <Calculator className="calculator-quick-icon relative h-[18px] w-[18px]" />
            </button>
