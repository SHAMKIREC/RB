...
export default function InlinePriceEditor({ value, onSave }) {
  ...
  return <div className="mt-2 rounded-lg border border-primary/30 bg-primary/5 p-2 text-left">
    ...
    <div className="mt-2 flex gap-2"><button type="button" onClick={async () => { if (await onSave(nextValue)) setEditing(false); }} className="rounded-md bg-primary px-2 py-1 text-[10px] font-bold text-white">Сохранить</button><button type="button" onClick={() => setEditing(false)} className="rounded-md border border-border px-2 py-1 text-[10px] font-bold">Отмена</button></div>
  </div>;
}
