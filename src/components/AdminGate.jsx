  ...
  return <div className="sticky top-0 z-50 border-b border-black/20 bg-[#242321]/95 text-white shadow-[0_12px_30px_-22px_rgba(0,0,0,.8)] backdrop-blur-xl">
    <div className="page-shell py-2.5 sm:py-3">
      <div className="flex flex-wrap items-center gap-2 sm:flex-nowrap sm:gap-3">
        <div className="flex h-11 shrink-0 items-center gap-2 rounded-xl border border-orange-400/35 bg-gradient-to-br from-orange-500/25 to-red-500/15 px-2.5 sm:h-12 sm:px-3">
          ...
          <span className="hidden leading-tight min-[390px]:block"><span className="block text-[10px] uppercase tracking-[.16em] text-white/70">Панель</span><span className="block text-xs font-black text-white">Владельца</span></span>
        </div>
        ...
        <nav aria-label="Разделы админки" className="order-3 flex min-w-0 basis-full gap-2 overflow-x-auto scroll-smooth pb-0.5 pt-0.5 [scroll-padding-inline:4px] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:order-none sm:flex-1 sm:basis-auto sm:pt-0">
          ...
          <Link to="/services" onClick={enableInlineEditMode} className="group relative inline-flex h-11 shrink-0 items-center justify-center gap-2 overflow-hidden rounded-xl border border-orange-400/55 bg-orange-500/15 px-3 text-xs font-black text-orange-100 transition-all duration-300 hover:-translate-y-0.5 hover:bg-orange-500/25 hover:text-white sm:h-12 sm:px-4" aria-label="Редактировать цены услуг" title="Редактировать цены услуг">
            <BadgeRussianRuble className="h-4 w-4"/><span className="whitespace-nowrap">Цены услуг</span>
          </Link>
        </nav>

        <div className="ml-auto flex shrink-0 gap-2 sm:ml-0">
          <Link to="/" aria-label="Открыть сайт" title="Открыть сайт" className="group inline-flex h-11 w-11 items-center justify-center rounded-xl border border-white/15 bg-white/[.08] text-white/90 transition-all duration-300 hover:-translate-y-0.5 hover:border-orange-400/55 hover:bg-white/[.14] hover:text-orange-200 sm:h-12 sm:w-auto sm:gap-2 sm:px-3">
          ...
          <button type="button" onClick={onExit} aria-label="Выйти из админки" title="Выйти" className="group inline-flex h-11 w-11 items-center justify-center rounded-xl border border-red-400/30 bg-red-500/15 text-red-100 transition-all duration-300 hover:-translate-y-0.5 hover:border-red-400/60 hover:bg-red-500/25 hover:text-white sm:h-12 sm:w-auto sm:gap-2 sm:px-3">
...
export default function AdminGate({ children }) {
  ...
  if (allowed) return <><AdminNavigation onExit={async () => { try { await endAdminSession(); setAllowed(false); } catch { setError('Не удалось завершить сессию. Попробуйте ещё раз.'); } }} /><div data-admin-content className="text-foreground dark:[&_p.text-muted-foreground]:text-slate-300 dark:[&_span.text-muted-foreground]:text-slate-300 dark:[&_label.text-muted-foreground]:text-slate-300">{children}</div></>;
  return (
    <section data-admin-login className="relative isolate min-h-[calc(100svh-4rem)] overflow-hidden bg-[#d9c1a3] pb-[env(safe-area-inset-bottom)] sm:min-h-[calc(100svh-5rem)] lg:min-h-[600px] xl:min-h-[620px]">
      ...
      <img src={adminLoginBackground} alt="" className="absolute inset-0 h-full w-full object-cover object-center xl:object-fill" />
      ...
      <div className="relative z-30 mx-auto flex min-h-[calc(100svh-4rem)] w-full items-start justify-center px-3 pb-8 pt-32 min-[480px]:px-6 min-[480px]:pt-36 sm:min-h-[calc(100svh-5rem)] sm:px-8 sm:pt-40 md:pt-40 lg:min-h-[600px] lg:pt-40 xl:min-h-[620px] xl:pt-44">
        <div className="relative flex min-h-[250px] w-full min-w-0 max-w-[350px] items-center justify-center text-slate-900 min-[480px]:min-h-[270px] min-[480px]:max-w-[410px] sm:min-h-[285px] sm:max-w-[470px] md:max-w-[500px] lg:min-h-[300px] lg:max-w-[540px] xl:min-h-[320px] xl:max-w-[620px]">
          ...
          <img src={adminLoginFrame} alt="" className="pointer-events-none absolute inset-0 z-10 h-full w-full object-fill" />
          <div className="relative z-20 w-full px-[12%] py-7 min-[480px]:py-8 sm:px-[12%] sm:py-9 lg:py-10 xl:py-12">
            ...
            {!configured ? <p className="mt-4 text-center text-sm text-destructive xl:mt-5">Административный доступ не настроен.</p> : <form onSubmit={async (event) => { event.preventDefault(); setError(''); try { const ok = await startAdminSession(pin); if (ok) setAllowed(true); else setError('Неверный PIN.'); } catch { setError('Не удалось выполнить вход. Попробуйте ещё раз.'); } }} className="mt-4 space-y-2 min-[480px]:space-y-2.5 sm:mt-5 sm:space-y-3 xl:mt-6 xl:space-y-3.5">
              <div className="relative"><input value={pin} onChange={(event) => setPin(event.target.value)} type="password" inputMode="text" autoCapitalize="none" autoCorrect="off" className="h-10 w-full rounded-xl border border-amber-300/70 bg-white px-4 pr-12 outline-none shadow-inner shadow-amber-900/5 transition-[border-color,box-shadow] duration-200 focus:border-primary focus:ring-2 focus:ring-primary/15 sm:h-11 xl:h-12" placeholder="PIN"/><Crown aria-hidden="true" className="pointer-events-none absolute right-4 top-1/2 h-5 w-5 -translate-y-1/2 text-amber-500/80" strokeWidth={1.8} /></div>
              <button className="group relative h-10 w-full overflow-hidden rounded-xl bg-primary text-sm font-bold text-white shadow-md shadow-orange-500/25 transition-[transform,box-shadow] duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-orange-500/35 sm:h-11 sm:text-base xl:h-12"><span aria-hidden="true" className="absolute inset-y-0 -left-1/3 w-1/3 skew-x-[-20deg] bg-gradient-to-r from-transparent via-white/20 to-transparent transition-transform duration-500 group-hover:translate-x-[430%]" /><span className="relative">👑 Войти как Король</span></button>
