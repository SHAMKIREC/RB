          ...
          {ITEMS.map((item, i) => {
            ...
            return (
              <motion.div
                ...
                className={`rb-card rb-card-action min-w-0 rounded-xl p-3 sm:rounded-2xl sm:p-6 ${i === 0 ? "sos-card-heartbeat relative border-primary/70" : ""}`}
              >
                <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-lg logo-gradient shadow-md shadow-orange-500/20 sm:mb-4 sm:h-12 sm:w-12 sm:rounded-xl">
                ...
                {i === 0 && (
                  <motion.span
                    ...
                    className="absolute right-3 top-3 bg-gradient-to-br from-[#FF6B35] to-[#FF3300] bg-clip-text text-xl font-black leading-none tracking-tight text-transparent drop-shadow-[0_5px_11px_rgba(255,80,0,0.34)] sm:right-6 sm:top-6 sm:text-[34px]"
                  >
                    24/7
                  </motion.span>
                )}
                <h3 className="mb-1.5 text-sm font-bold leading-tight text-foreground sm:mb-2 sm:text-base">{item.title}</h3>
                <p className="text-xs leading-snug text-muted-foreground sm:text-sm sm:leading-relaxed">{item.desc}</p>
              </motion.div>
            );
          })}
        </div>
