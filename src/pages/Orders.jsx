...
export default function Orders() {
  ...
  return <div className="page-shell py-7 sm:py-10">
    ...
    {loading ? <div className="rounded-2xl border border-dashed border-border px-6 py-14 text-center text-muted-foreground">Загрузка...</div>
          ...
          {orders.map((order) => <article key={order.id} className="rb-card group flex flex-col overflow-hidden rounded-2xl">
            ...
            <div className="flex flex-1 flex-col p-5">
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                <span className="rounded-xl bg-secondary p-3 text-xs"><BriefcaseBusiness className="mb-2 h-4 w-4 text-primary" /><b className="block">{order.worksCount ?? order.selectedWorks?.length ?? 0}</b><span className="text-muted-foreground">видов работ</span></span>
                <span className="rounded-xl bg-secondary p-3 text-xs"><Clock3 className="mb-2 h-4 w-4 text-primary" /><b className="block">{order.preferredDeadline || 'По договорённости'}</b><span className="text-muted-foreground">срок</span></span>
              </div>
              <div className="mt-auto grid grid-cols-2 gap-2 pt-5"><Link to={`/orders/${order.id}`} className="rounded-xl border border-primary/30 py-2.5 text-center text-xs font-bold hover:bg-primary/5">Смотреть смету</Link><button onClick={() => setResponseOrder(order)} className="rounded-xl bg-primary py-2.5 text-xs font-bold text-white"><MessageCircle className="mr-1 inline h-4 w-4" />Откликнуться</button></div>
            </div>
          </article>)}
        </div>
      ...
      </> : <div className="rounded-2xl border border-dashed border-border px-6 py-14 text-center text-muted-foreground">Активных заказов пока нет.</div>}
    {responseOrder && <OrderResponseModal number={responseOrder.number} onClose={() => setResponseOrder(null)} />}
  </div>;
}
