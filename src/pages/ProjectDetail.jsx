...
import { absoluteUrl, breadcrumbSchema, compactDescription } from '../lib/seo';
...
const projectPhotoAlt = (project, stage = '') => {
  const place = project.location || 'Саратов';
  const suffix = stage ? `, ${stage.toLowerCase()}` : '';
  return `${project.title} — ремонт и строительные работы, ${place}${suffix} | РБ Решаем Быстро`;
};
...
export default function ProjectDetail() {
  ...
  const seoTitle = `${project.title} — ремонт и строительство в Саратове | РБ`;
  const seoDescription = compactDescription(project.description, `Завершённый проект «${project.title}» в Саратове: выполненные ремонтные и строительные работы, фотографии объекта и итоговая смета.`);
  const canonicalPath = `/projects/${encodeURIComponent(project.id)}`;
  const imageAlt = projectPhotoAlt(project);
  const imageSchemas = photos.slice(0, 12).map((photo, index) => ({
    '@context': 'https://schema.org',
    '@type': 'ImageObject',
    contentUrl: photo,
    url: photo,
    caption: `${project.title}: фото ${index + 1}, ремонт и строительные работы, ${project.location || 'Саратов'}`,
  ...
  const projectSchema = {
    ...
    image: photos.slice(0, 12),
    about: ['ремонт', 'строительные работы', 'Саратов'],
    locationCreated: project.location || 'Саратов',
    provider: { '@id': 'https://www.rb-24.ru/#business', name: 'РБ Решаем Быстро' },
  };
  const schemas = [
    breadcrumbSchema([{ name: 'Главная', path: '/' }, { name: 'Проекты', path: '/projects' }, { name: project.title, path: canonicalPath }]),
    projectSchema,
    ...imageSchemas,
  ];

  return <div className="page-shell py-10 sm:py-16">
    <SeoHead title={seoTitle} description={seoDescription} canonicalPath={canonicalPath} image={cover || undefined} imageAlt={imageAlt} type="article" schemas={schemas} />
    ...
    {cover && <button onClick={() => setOpenIndex(photos.indexOf(cover))} className="mt-6 block h-[min(48vw,420px)] w-full rounded-2xl bg-secondary/60 p-4"><img src={cover} alt={projectPhotoAlt(project)} width="1200" height="800" className="h-full w-full object-contain object-center" /></button>}
