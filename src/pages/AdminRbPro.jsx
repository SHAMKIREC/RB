...
import { Copy, KeyRound, ShieldCheck, ToggleLeft, ToggleRight } from 'lucide-react';
import AdminGate from '../components/AdminGate';
import { createRbProCode, listRbProCodes, setRbProCodeActive } from '../lib/rbProAccess';
  ...
  return <main className="page-shell py-8 sm:py-12">
    <div className="mx-auto max-w-3xl">
      ...
      <section className="mt-7 rounded-3xl border border-border bg-card p-5 shadow-sm sm:p-7">
        <div className="flex gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-green-600" /><p className="text-sm text-muted-foreground">После одобрения мастера в Telegram создайте для него отдельный код. Сам код показывается только один раз. В базе хранится только его хэш.</p></div>
        ...
        {newCode && <div className="mt-4 rounded-2xl border border-green-300 bg-green-50 p-4 text-green-950 dark:border-green-700 dark:bg-green-950/30 dark:text-green-100"><p className="text-xs font-bold uppercase tracking-wider">Отправьте мастеру сейчас</p><div className="mt-2 flex items-center gap-2"><code className="min-w-0 flex-1 break-all text-lg font-black">{newCode}</code><button type="button" onClick={copy} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-green-600 text-white"><Copy className="h-4 w-4" /></button></div><p className="mt-2 text-xs opacity-75">После закрытия страницы этот код повторно посмотреть нельзя. При необходимости создайте новый.</p></div>}
      ...
      <section className="mt-5 rounded-3xl border border-border bg-card p-5 sm:p-7">
        ...
        <div className="mt-4 space-y-3">
          {codes.length ? codes.map((item) => <div key={item.id} className="flex items-center gap-3 rounded-2xl border border-border bg-background p-4">
            <div className="min-w-0 flex-1"><p className="font-black">{item.label || 'Без пометки'}</p><p className="mt-1 text-xs text-muted-foreground">Создан: {dateText(item.created_at)} · Последний вход: {dateText(item.last_used_at)} · Сессий: {item.active_sessions || 0}</p></div>
